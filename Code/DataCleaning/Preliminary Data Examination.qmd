...
try <- Cape_May_2 |>
  ...
  select(Date, Species, Duration, n_individ, total_n, duration_min_day)

try$year <- lubridate::year(try$Date)
try$month <- lubridate::month(try$Date)
try$julian <- lubridate::yday(try$Date) 

### code above works!! DO NOT GET RID OF UNTIL SURE CODE BELOW WORKS!!
...
###time to clean now
# sliced out row 5 as well, it is a row of totals.
...
write.csv(Cape_may_clean, here("Data","Processed Data","CM_clean.csv"), row.names = FALSE) #row.names=FALSE means please don't make X column where it has numbered the rows
